...
unsigned sjf_scheduler(struct process_node *p)
{
	...
	r=initialize_ready_queue(p,r,4);
	...
	do
	{
		...
		while(temp_queue!=NULL)
		{
			...
			//if(b->next!=NULL)
			//{
				if(b->exit_code==3)			//I/O burst			
				{
					temp_process->io_time+=b->burst_duration;
			...
			r=switch_process_to_ready_queue(p,r,&i,4);
			r=switch_process_to_ready_queue(p,r,&o,4);
			r=switch_process_to_ready_queue(p,r,&d,4);
display_task_manager(-1,p,i,o,d,r,0);///
			temp_queue=r;
		}

//UNCOMMENT LATER		display_task_manager(-1,p,i,o,d,r,0);
